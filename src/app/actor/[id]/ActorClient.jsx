    ...
    <main className='min-h-screen bg-gradient-to-b from-primary to-secondary pt-16'>
      <div className='container mx-auto px-4 py-8'>
        <div className='grid md:grid-cols-[300px,1fr] gap-8'>
          ...
          >
            <div>
              <h1 className='text-4xl font-bold mb-2'>{actorInfo.name}</h1>
              ...
              <div className='space-y-4'>
                {sortedCredits?.map((credit) => (
                  <Link
                    key={`${credit.id}-${credit.credit_id}`}
                    href={`/media/player/${credit.media_type.includes('tv') ? 'show' : 'movie'}/${credit.id}`}
                    className='block glass-effect p-4 rounded-lg hover:bg-white/5 transition-colors'
                  >
                    <div className='flex items-center gap-4'>
                      {credit.poster_path ? (
                        ...
                        <div className='w-16 h-24 bg-gray-800 flex items-center justify-center rounded'>
